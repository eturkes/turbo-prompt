...
- Prompt history fingerprints full compiled text, marks non-exact legacy entries, preserves exact copied snapshots across workflow changes + undo, exposes every storage-retained entry, and deduplicates exact output only.
- Clipboard success is bound to a draft revision/operation token; edits invalidate pending feedback and history insertion.
- Narrow UI = labeled stacked fields with wrapping values + a reserved copy footer; desktop retains inline prose. Each field is one Tab stop; Delete/Backspace clears.
- Indexed instruction metadata carries its directory scope. Secret directory segments and unsafe persisted paths/colors are rejected before display.
